...
func (i Int64) Less(than pkg.Item) bool {
	l, ok := than.(Int64)
	if !ok {
		return false
	}
...
func TestSerialize(t *testing.T) {
	btree := pkg.NewBTree()
	btree.Insert(pkg.IntItem(1))
	b, err := pkg.SerializeBTree(btree)
	if err != nil {
		log.Fatal(err)
	}

	newTree, err := pkg.DeserializeBTree(b)
	if err != nil {
		log.Fatal(err)
	}

	item := newTree.Top.Items[0].(pkg.IntItem)
	assert.Equal(t, item, pkg.IntItem(1))
}
